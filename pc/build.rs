...
fn extract_file(zip: &ZipArchive, file: &str) {
	...
	let mut save_to = File::create(get_output_path(&file))
		.expect(&format!("failed to create file {file}"));
	io::copy(&mut reader, &mut save_to).expect(&format!("failed to write file {file}"));
}

#[cfg(target_os = "windows")]
fn get_output_path(file: &str) -> PathBuf {
	let output_path = PathBuf::from(file);
	PathBuf::from(env::var("OUT_DIR").unwrap()).join(output_path.file_name().unwrap())
}
...
fn main() {
	...
	#[cfg(target_os = "windows")]
	{
		...
		if files.iter().any(|f| !get_output_path(f).exists()) {
